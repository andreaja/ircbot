...
def location(name, username):
   name = urllib.quote(name.encode('utf-8'))
   uri = 'http://ws.geonames.org/searchJSON?q=%s&maxRows=1&username=%s' % (name, username)
   for i in xrange(10):
...
def code(phenny, search):
   ...
   if search.upper() in [loc[0] for loc in data]:
      ...
      name, country, latitude, longitude = location(search, phenny.config.weather_username)
...
def f_weather(self, origin, match, args):
   ...
   if not hasattr(self.config, "weather_username"):
      return self.msg(origin.sender, 'Weather is not configured')
